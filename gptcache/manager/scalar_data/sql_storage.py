...
class SQLStorage(CacheStorage):
    ...
    def get_data_by_id(self, key: int) -> Optional[CacheData]:
        with self.Session() as session:
            ...
            _emb = np.frombuffer(qs.embedding_data, dtype=np.float32)
            _embed_dim = len(_emb)
            return CacheData(
                question=qs.question if not deps else Question(qs.question, res_deps),
                answers=res_ans,
                embedding_data=_emb,
                context_data=np.frombuffer(qs.context_data, dtype=np.float32).reshape(-1, _embed_dim),
                cur_id=qs.cur_id,
                pre_id=qs.pre_id,
        ...
        with self.Session() as session:
            ...
            for qs in results:
                ...
                cache_data = CacheData(
                    ...
                    context_data=np.frombuffer(qs.context_data, dtype=np.float32).reshape(-1, len(np.frombuffer(qs.embedding_data, dtype=np.float32))) if qs.context_data else None,
                    cur_id=qs.cur_id,
                    pre_id=qs.pre_id,
                    create_on=qs.create_on,
                    last_access=qs.last_access,
